...
export async function loader({ params, request }: LoaderFunctionArgs) {
  try {
    ...
    const { data } = await api.get(`/products/sx/${productId}`);
...
export default function EditProduct() {
  ...
  return (
    <div>
      ...
      >
        ...
        <AlertDescription className="text-green-700">
          <div className="flex items-center gap-2">
            <span>
              Congratulations! Your product has been added. View it at
            </span>
            <a
              href={`/${productId}`}
      ...
      <ClientOnly>
        {() => {
          return <ProductForm isEdit={true} productInfo={product} />;
        }}
      </ClientOnly>
    </div>
  );
